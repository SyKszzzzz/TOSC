...
def create_model(cfg: DictConfig, *args: List, **kwargs: Dict) -> nn.Module:
    ...
    """
    if 'diffuser' in cfg:
        return create_diffuser(cfg, *args, **kwargs) # DDPM with eps_model(UNetModel)
...
def create_diffuser(cfg: DictConfig, *args: List, **kwargs: Dict) -> nn.Module:
    ...
    eps_model = MODEL.get(cfg.model.name)(cfg.model, *args, **kwargs)  #  UNetModel

    ## if the task has observation, then pass it to diffuser
    has_obser = cfg.task.has_observation if 'has_observation' in cfg.task else False  # False
    diffuser  = DIFFUSER.get(cfg.diffuser.name)(eps_model, cfg.diffuser, has_obser, *args, **kwargs) # DDPM, has_obser = False
