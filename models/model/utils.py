...
def timestep_embedding(timesteps, dim, max_period=10000, repeat_only=False):
    ...
    if len(timesteps.shape) == 2:
        # 分别计算 t 和 t-r 的 embedding，然后相加融合
        # 对应 MeanFlow 论文中的 embedding summation 策略
        emb1 = timestep_embedding(timesteps[:, 0], dim, max_period)
        emb2 = timestep_embedding(timesteps[:, 1], dim, max_period)
        return emb1 + emb2
    if not repeat_only:
        half = dim // 2
        freqs = torch.exp(
